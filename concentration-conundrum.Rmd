...
#### Table 5
**Table 5:** the 10 most common compounds in EIPAAB versus occurrence and detection frequency in Wilkson
...
wilkson_counts <- eipaab_wilkson %>% 
  dplyr::filter(source == "wilkson") %>% 
  dplyr::group_by(compound_name) %>% 
  dplyr::reframe(n_wilkson = sum(n_units_est, na.rm = TRUE)) %>% 
  ungroup() %>% 
  tidyr::complete(., compound_name)

wilkson_counts_pos <- eipaab_wilkson %>% 
  dplyr::filter(source == "wilkson") %>% 
  dplyr::filter(value > 0) %>% 
  dplyr::group_by(compound_name) %>% 
  dplyr::reframe(n_postive_detection = length(unique_row_id)) %>% 
...
eipaab_wilkson_counts <- left_join(eipaab_counts, wilkson_counts, by = "compound_name") %>% 
  left_join(., wilkson_counts_pos, by = "compound_name") %>% 
  dplyr::mutate(n_postive_detection = if_else(is.na(n_postive_detection), 0, n_postive_detection),
                rank_detection_wilkson = rank(n_postive_detection, ties.method = "min"),
                rank_eipaab = rank(n_eipaab, ties.method = "min"),
                rank_detection_dif = rank_detection_wilkson-rank_eipaab,
                log_n_detections_wilkson = if_else(n_postive_detection > 0, log(n_postive_detection), n_postive_detection),
                log_n_eipaab = log(n_eipaab),
                prop_postive = n_postive_detection/n_wilkson) %>% 
  dplyr::arrange(desc(rank_detection_dif)) %>% 
  dplyr::mutate(rank_detection_dif_order = 1:nrow(.))

eipaab_wilkson_counts %>% 
  dplyr::arrange(rank_detection_dif_order) %>% 
  dplyr::slice(1:10) %>% 
  dplyr::mutate(compound_name = sentence_case(compound_name)) %>% 
  dplyr::select(compound_name, n_eipaab, n_postive_detection, rank_detection_dif_order) %>% 
  dplyr::rename(Name = compound_name, "EIPAAB tests" = n_eipaab, "Detections" = n_postive_detection, "Rank order difference in detections and testing" = rank_detection_dif_order) %>% 
  gt() %>% 
  cols_align(
    align = "center", 
    columns = everything()
